...
(function(){
  ...
  function fixGeneratedPostLinks(){
    if (document.body.classList.contains('theme-toronto') || document.body.classList.contains('theme-hiking')) return;
    if (!/\/posts\//.test(window.location.pathname)) return;
    const links = Array.from(document.querySelectorAll('.button-row a.btn'));
    links.forEach((link) => {
      const label = (link.textContent || '').trim().toLowerCase();
      if (label === 'return home') link.setAttribute('href', '../../../index.html');
      if (label === 'back to section') link.setAttribute('href', '../../index.html');
    });
  }
