  ...
  async function loadPosts(){
    ...
    try {
      ...
      holder.innerHTML = posts.map((post) => {
        ...
        return `<article class="content-card ${mode === 'album' ? 'album-card' : 'post-card'}">${media}<div class="stack"><h3>${esc(post.title)}</h3>${post.subtitle ? `<p class="lead markdown-inline">${renderMarkdownInline(post.subtitle)}</p>` : ''}<div class="tag-row">${tag}${mode==='piano' && post.youtubeUrl ? '<span class="tiny-tag">YouTube</span>' : ''}</div>${mode === 'album' ? '' : `<div class="post-excerpt markdown-body">${renderMarkdownBlock(excerpt)}</div>`}<div class="cta-inline"><a class="mini-link" href="posts/${esc(post.slug)}/index.html">${cta}</a></div></div></article>`;
      }).join('');
    } catch (err){
      holder.className='';
