...
function buildPostHtml(sectionKey, post){ const section = SECTIONS[sectionKey]; const mediaBlock = (post.media || []).length ? `<section class="content-card stack"><div class="media-grid">${(post.media || []).map((m) => m.type && m.type.startsWith('video') ? `<figure><video controls playsinline src="media/${esc(m.filename)}"></video></figure>` : `<figure><img src="media/${esc(m.filename)}" alt="${esc(post.title)}" /></figure>`).join('')}</div></section>` : ''; const youtubeBlock = (sectionKey === 'piano' && post.youtubeUrl && youtubeId(post.youtubeUrl)) ? `<section class="content-card stack"><div class="video-embed"><iframe src="https://www.youtube.com/embed/${youtubeId(post.youtubeUrl)}" title="${esc(post.title)}" loading="lazy" allowfullscreen></iframe></div></section>` : ''; const rating = Number(post.rating || 0); const ratingBlock = section.reviewScale && rating ? `<section class="content-card stack"><div class="rating-line compact"><div class="overall-wrap"><span class="eyebrow-label">Rating</span><span class="stars-wrap overall-stars"><span class="stars">${stars(rating)}</span><span class="rating-mini-number">${rating.toFixed(1)}</span></span></div></div></section>` : ''; const main = `<section class="hero-panel hero-title-card stack" style="--hero-image:url('../../../assets/${section.hero_image}')"><h1>${esc(post.title)}</h1>${post.subtitle ? `<p class="lead">${renderMarkdownInline(post.subtitle)}</p>` : ''}<div class="tag-row"><span class="tiny-tag">${prettyDate(post.date)}</span></div></section>${ratingBlock}${youtubeBlock}${mediaBlock}<section class="content-card stack"><div class="text-panel markdown-body">${renderMarkdownBlock(post.body)}</div></section><div class="button-row"><a class="btn" href="../../index.html">Return Home</a><a class="btn" href="../index.html">Back to section</a></div>`; return wrapHtml({ title:`${post.title} | Nihilists Take Their Coffee Black`, bodyClass:section.theme, prefix:'../../../', active:sectionKey, mainContent:main }); }
