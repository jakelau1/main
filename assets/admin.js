...
const postRatingWrap = $('postRatingWrap');
...
[reviewCleanliness, reviewAesthetics, reviewFacilities, hikingDifficulty, hikingBeauty, postRating].forEach((el) => el.innerHTML = ratingOptions());
postRating.value='4.0';
...
function showFields(){ const value=sectionSelect.value; const toilet=value==='where-to-poop-toronto' || value==='where-to-poop-hong-kong'; const hiking=value==='hiking-trails-hong-kong'; const piano=value==='piano'; const mediaReview=value==='book-and-media-reviews'; postFields.classList.toggle('hidden-block', toilet || hiking); toiletFields.classList.toggle('hidden-block', !toilet); hikingFields.classList.toggle('hidden-block', !hiking); youtubeWrap.classList.toggle('hidden-block', !piano); postRatingWrap.classList.toggle('hidden-block', !mediaReview); }
...
function menuHtml(prefix, active){
  const reviewsActive = ['where-to-poop-toronto','where-to-poop-hong-kong','hiking-trails-hong-kong','book-and-media-reviews'].includes(active);
  const blogsActive = ['reflections-from-a-mountaintop','philosophical-yap-sessions','reflections-of-a-casual-bouldering-enthusiast','piano','doodling','cooking','games-and-the-meaning-of-life'].includes(active);
  const albumsActive = ['kyle-the-dog','photos'].includes(active);
  function link(href,label,key){ return `<a class="${active===key ? 'side-link active' : 'side-link'}" href="${prefix}${href}">${label}</a>`; }
  function sub(href,label,key){ return `<a class="${active===key ? 'side-sub-link active' : 'side-sub-link'}" href="${prefix}${href}">${label}</a>`; }
  const openBlogs = blogsActive ? ' open' : '';
  const openReviews = reviewsActive ? ' open' : '';
  const openAlbums = albumsActive ? ' open' : '';
  return `<aside class="sidebar" id="siteSidebar"><div class="sidebar-inner"><a class="site-brand" href="${prefix}index.html"><span class="brand-mark">☕</span><span class="brand-stack"><strong>Nihilists Take Their Coffee Black</strong></span></a><nav class="side-nav" aria-label="Site sections">${link('index.html','Home','home')}<details class="side-group"${openBlogs}><summary><span class="group-title">Blogs</span><span class="group-note">essays & posts</span></summary><div class="side-sub-links">${sub('reflections-from-a-mountaintop/index.html','Reflections from a Mountaintop','reflections-from-a-mountaintop')}${sub('philosophical-yap-sessions/index.html','Philosophical Yap Sessions','philosophical-yap-sessions')}${sub('reflections-of-a-casual-bouldering-enthusiast/index.html','Reflections of a Casual Bouldering Enthusiast','reflections-of-a-casual-bouldering-enthusiast')}${sub('piano/index.html','Piano','piano')}${sub('doodling/index.html','Doodling','doodling')}${sub('cooking/index.html','Cooking','cooking')}${sub('games-and-the-meaning-of-life/index.html','Games and the Meaning of Life','games-and-the-meaning-of-life')}</div></details><details class="side-group"${openReviews}><summary><span class="group-title">Reviews</span><span class="group-note">routes & rankings</span></summary><div class="side-sub-links">${sub('hiking-trails-hong-kong/index.html','Hiking Trails: Hong Kong','hiking-trails-hong-kong')}${sub('book-and-media-reviews/index.html','Book and Media Reviews','book-and-media-reviews')}${sub('where-to-poop-toronto/index.html','Where to Poop: Toronto','where-to-poop-toronto')}${sub('where-to-poop-hong-kong/index.html','Where to Poop: Hong Kong','where-to-poop-hong-kong')}</div></details><details class="side-group"${openAlbums}><summary><span class="group-title">Albums</span><span class="group-note">visual sections</span></summary><div class="side-sub-links">${sub('kyle-the-dog/index.html','Kyle the Dog','kyle-the-dog')}${sub('photos/index.html','Photography','photos')}</div></details>${link('hd-hen/index.html','HD Hen','hd-hen')}</nav></div></aside>`; }
...
function buildPostHtml(sectionKey, post){ const section = SECTIONS[sectionKey]; const mediaBlock = (post.media || []).length ? `<section class="content-card stack detail-media-card"><div class="media-grid">${(post.media || []).map((m) => m.type && m.type.startsWith('video') ? `<figure><video controls playsinline src="media/${esc(m.filename)}"></video></figure>` : `<figure><img src="media/${esc(m.filename)}" alt="${esc(post.title)}" /></figure>`).join('')}</div></section>` : ''; const youtubeBlock = (sectionKey === 'piano' && post.youtubeUrl && youtubeId(post.youtubeUrl)) ? `<section class="content-card stack detail-media-card"><div class="video-embed"><iframe src="https://www.youtube.com/embed/${youtubeId(post.youtubeUrl)}" title="${esc(post.title)}" loading="lazy" allowfullscreen></iframe></div></section>` : ''; const rating = Number(post.rating || 0); const ratingBlock = section.reviewScale && rating ? `<section class="content-card stack detail-rating-card"><div class="rating-line compact"><div class="overall-wrap"><span class="eyebrow-label">Rating</span><span class="stars-wrap overall-stars"><span class="stars">${stars(rating)}</span><span class="rating-mini-number">${rating.toFixed(1)}</span></span></div></div></section>` : ''; const main = `<section class="hero-panel hero-title-card stack" style="--hero-image:url('../../../assets/${section.hero_image}')"><h1>${esc(post.title)}</h1>${post.subtitle ? `<p class="lead">${renderMarkdownInline(post.subtitle)}</p>` : ''}<div class="tag-row"><span class="tiny-tag">${prettyDate(post.date)}</span></div></section>${ratingBlock}${youtubeBlock}${mediaBlock}<section class="content-card stack detail-text-card"><div class="text-panel markdown-body">${renderMarkdownBlock(post.body)}</div></section><div class="button-row"><a class="btn" href="../../../index.html">Return Home</a><a class="btn" href="../../index.html">Back to section</a></div>`; return wrapHtml({ title:`${post.title} | Nihilists Take Their Coffee Black`, bodyClass:section.theme, prefix:'../../../', active:sectionKey, mainContent:main }); }
...
function buildHongKongDetailHtml(){ return wrapHtml({ title:'Hong Kong Toilet Review | Nihilists Take Their Coffee Black', bodyClass:'theme-hk', prefix:'../../../', active:'where-to-poop-hong-kong', mainContent:`<section class="hero-panel hero-title-card stack" style="--hero-image:url('../../../assets/2.jpg');--hero-position:center 24%" id="torontoDetailHero"></section><section class="detail-card stack" id="torontoDetailBody"></section>`, extraScripts:'<script src="../../../assets/toronto.js"></script>' }); }
...
$('downloadHongKongReviewsJson').addEventListener('click', () => { const blob = new Blob([JSON.stringify(mergedHongKongReviews(), null, 2)], {type:'application/json'}); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'hong-kong-reviews.json'; a.click(); URL.revokeObjectURL(a.href); });
...
$('downloadBundle').addEventListener('click', async () => {
  if(!queued.length) return alert('Queue at least one entry first.');
  if(!window.JSZip) return alert('Zip library failed to load.');
  try {
    const zip = new JSZip();
    const posts = mergedPosts();
    const reviews = mergedReviews();
    const hongKongReviews = mergedHongKongReviews();
    const hiking = mergedHiking();
    if(queued.some((item) => item.kind === 'post')){
      zip.file('content/posts.json', JSON.stringify(posts, null, 2));
      queued.filter((item) => item.kind === 'post').forEach((item) => {
        const mediaFiles = Array.isArray(item.media) ? item.media : [];
        const post = {
          title:item.title,
          slug:item.slug,
          ...
          media:mediaFiles.map((file) => ({ filename:file.name, type:file.type || '' }))
        };
        zip.file(`${item.section}/posts/${item.slug}/index.html`, buildPostHtml(item.section, post));
        mediaFiles.forEach((file) => zip.file(`${item.section}/posts/${item.slug}/media/${file.name}`, file));
      });
    }
    if(queued.some((item) => item.kind === 'toilet' && item.section === 'where-to-poop-toronto')){
      zip.file('content/toronto-reviews.json', JSON.stringify(reviews, null, 2));
      queued.filter((item) => item.kind === 'toilet' && item.section === 'where-to-poop-toronto').forEach((item) => {
        zip.file(`where-to-poop-toronto/reviews/${item.slug}/index.html`, buildTorontoDetailHtml());
      });
    }
    if(queued.some((item) => item.kind === 'toilet' && item.section === 'where-to-poop-hong-kong')){
      zip.file('content/hong-kong-reviews.json', JSON.stringify(hongKongReviews, null, 2));
      queued.filter((item) => item.kind === 'toilet' && item.section === 'where-to-poop-hong-kong').forEach((item) => {
        zip.file(`where-to-poop-hong-kong/reviews/${item.slug}/index.html`, buildHongKongDetailHtml());
      });
    }
    if(queued.some((item) => item.kind === 'hiking')){
      zip.file('content/hiking-trails-hong-kong.json', JSON.stringify(hiking, null, 2));
      queued.filter((item) => item.kind === 'hiking').forEach((item) => {
        zip.file(`hiking-trails-hong-kong/reviews/${item.slug}/index.html`, buildHikingDetailHtml());
      });
    }
    zip.file('batch-manifest.txt', queued.map((item) => item.kind === 'toilet' ? `${item.section}/reviews/${item.slug}/index.html` : item.kind === 'hiking' ? `hiking-trails-hong-kong/reviews/${item.slug}/index.html` : `${item.section}/posts/${item.slug}/index.html`).join('\n'));
    const blob = await zip.generateAsync({type:'blob'});
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = 'content-studio-bundle.zip';
    a.click();
    setTimeout(() => URL.revokeObjectURL(a.href), 1000);
  } catch (error) {
    console.error('Bundle generation failed', error);
    alert(`Bundle generation failed: ${error && error.message ? error.message : 'Unknown error'}`);
  }
});
loadBase();
