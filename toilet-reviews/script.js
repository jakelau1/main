const REVIEWS = [
  {
    ...
    "facilities": 2.5,
    "date": "2026-03-27",
    "summary": "Clean enough for No. 1; barely acceptable for No. 2. Dim lighting, noticeable odor. Suitable for emergencies, but shouldn't be a first choice for anyone.",
    "otherNotes": "The diner is located in the basement, so you'll have to go down a flight of stairs."
  },
  {
...
function reviewTags(review) {
  const tags = [];
  if (review.featured) {
    tags.push(`<span class="tag featured-tag">Featured Toilet</span>`);
  }
  tags.push(
    ...[
    ...
    ].map(([label, value]) => `<span class="tag"><strong>${escapeHtml(label)}:</strong> ${escapeHtml(value)}</span>`)
  );
  return tags.join('');
}
...
function getRootPrefix() {
  return normalizeBasePath(document.body.dataset.rootPrefix || '');
}
...
function renderSideMenu() {
  ...
  nav.innerHTML = `
    <a class="side-link" href="${rootPrefix}index.html">Home</a>
    <a class="side-link" href="${rootPrefix}philosophical-yap-sessions/">Philosophical yap sessions</a>
    <a class="side-link" href="${rootPrefix}reflections-of-a-casual-bouldering-enthusiast/">Reflections of a Casual Bouldering Enthusiast</a>
    <a class="side-link" href="${rootPrefix}book-and-media-reviews/">Book and Media Reviews</a>
    <a class="side-link" href="${rootPrefix}photos/">Photos</a>
    <a class="side-link" href="${rootPrefix}kyle-the-dog/">Kyle the Dog</a>
    <div class="side-nav-group is-open">
      <button class="side-group-toggle" type="button" aria-expanded="true" aria-controls="menuReviewsSubnav">
        <span>Where to Poop: Toronto</span>
        <span class="chevron" aria-hidden="true">▾</span>
      </button>
      <div class="side-sub-links" id="menuReviewsSubnav">
        <a class="side-sub-link" href="${basePath}index.html">Home</a>
        <a class="side-sub-link" href="${basePath}reviews/">All reviews</a>
        ${reviewLinks}
        ${includeAdmin ? `<a class="side-sub-link" href="${basePath}admin/">Admin</a>` : ''}
      </div>
    </div>
    <a class="side-link" href="${rootPrefix}where-to-poop-hong-kong/">Where to Poop: Hong Kong</a>
    <a class="side-link" href="${rootPrefix}hd-hen/">HD Hen</a>
  `;
    ...
    <article class="detail-card">
      <div class="detail-header">
        <div>
          ...
          ${review.featured ? '<div class="detail-featured-wrap"><span class="tag featured-tag detail-featured-tag">Featured Toilet</span></div>' : ''}
        </div>
